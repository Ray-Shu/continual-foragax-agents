"""
tppo.py -- minimal PPO training loop for the sliding-window Transformer-PPO agent.

This is a DELIBERATELY STRIPPED analogue of rtu_ppo.py. It keeps ONLY the
foundational PPO information flow so you can see, end to end, how data moves
between the environment, the rollout buffer, and the gradient update -- and so
you can confirm the TransformerPPO network is compatible with what the training
loop needs from an architecture. The research-y METRICS in rtu_ppo.py remain
removed: plasticity / sow() probes, NTK & churn, weight norm/drift, spectral &
L2-init regularisation, shrink-and-perturb, last-layer reset, and video capture.
The ml_instrumentation / experiment-framework scaffolding, however, IS wired in
(see main()): tppo takes the same `-e <exp.json> -i <idx>` CLI as rtu_ppo.py and
writes the same results.db + data/{idx}.npz artifacts (rewards, per-update
losses, and the foragax info columns pos/biome_id/object_collected_id/
biome_regret/biome_rank), so the standard plotting pipeline reads tppo runs too.
The columns that need architecture-specific instrumentation (NTK, plasticity
probes, churn, weight norm/drift) are intentionally NOT emitted.

The one structural change vs rtu_ppo.py: the carried state between steps is a
TOKEN WINDOW, not an RTU hidden state. Where rtu_ppo threads `hstate`, this
threads `window` (the last T tokens). This is the sliding-window (v1)
implementation -- correct but memory/compute-heavy on purpose, so you have a
baseline to measure the optimised version against.

rtu_ppo.py  ->  tppo.py  map
----------------------------------------------------------------------------
  agent_step / env_step        ->  _rollout_step (inside collect_rollout)
  Transition                   ->  Transition (obs is now a token tuple)
  hstate (RTU carry)           ->  window  (last T tokens, per obs component)
  calculate_gae                ->  calculate_gae (unchanged)
  create_minibaches            ->  build_windows + the shuffle/reshape in ppo_update
  loss_fn                      ->  loss_fn (PPO clip loss only, no reg terms)
  update_minbatch/epoch/step   ->  ppo_update (two nested lax.scans)
  experiment / experiment_step ->  train (outer Python loop, one jit per phase)

NOTE (compatibility): the update differentiates w.r.t. the `nnx.Param` leaves
returned by `nnx.split(model, nnx.Param, ...)`. EmbeddingNet currently wraps its
lazily-created Linear in `nnx.data(...)`, which tells nnx "this is data, not a
trainable submodule". If the embedding weights end up in `rest` instead of
`params`, they will NOT be trained. The param-count print in `train()` is there
to catch that -- if it looks too small, change EmbeddingNet to hold a plain
`nnx.Linear` (nnx auto-detects its Params) rather than `nnx.data(nnx.Linear(...))`.
"""
